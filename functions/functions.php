 ...
 * @return string
 */
function post($url, $postdata = '', array $opts = null, $sign = true)
{

    if (isset($postdata['key']) && $sign) {
        $key             = $postdata['key'];
 ...
 * @return boolean
 */
function is_real_trade($type = 1)
{
    return $type == service('config')->get('mt4.type.real');
}
 ...
 * @return boolean
 */
function is_demo_trade($type = 1)
{
    return $type == service('config')->get('mt4.type.demo');
}
 ...
 */
function mask($data, $start, $end, $replace = '*', $repeat = 2)
{
    $prefix = substr($data, 0, $start);
    $suffix = substr($data, $end);
    return $prefix . str_repeat($replace, $repeat) . $suffix;
}
...
function get_cookie_host()
{
    return '.' . join('.', array_slice(explode('.', HTTP_HOST), -2));
}

function set_cookie_with_host($key, $value, $lifetime = 86400, $path = '/', $domain = '')
{
    setcookie($key, $value, time() + $lifetime, $path, $domain ? $domain : get_cookie_host());
    $_COOKIE[$key] = $value;
}
